...
{
    public class MyoConnection : MonoBehaviour
    {
        ...
        private DeviceInformation _myoDeviceInformation;
...
#if WINDOWS_UWP
        public bool ConnectingOrConnected { get { return Connected || _myoBle != null || _myoDeviceInformation != null; } }
#else
        public bool ConnectingOrConnected { get { return Connected; } }
#endif
        ...
        public async Task<bool> ConnectKnownDevice(DeviceInformation deviceInfo)
        {
            _myoDeviceInformation = deviceInfo;
            ...
            if (bluetoothLeDevice != null)
            {
                ...
                if (services.Services.Single(s => s.Uuid == ControlServiceUUID) != null)
                {
                    ...
                    var connected = await ConnectToMyoBle(bluetoothLeDevice);
                    if (!connected)
                    {
                        _myoDeviceInformation = null;
                    }
                    return connected;
                }
            }
            ...
            {
                Debug.LogError("Unable to connect to Myo!");
            }
            _myoDeviceInformation = null;
            return false;
        }

        public async Task<bool> ConnectToMyoBle(BluetoothLEDevice bluetoothLeDevice)
        {
            ...
            try
            {
                ...
                EnqueueOnMainThread(() => {
                    Connected = true;
                    OnMyoConnected?.Invoke(); 
                });
                EnableSensorUpdates();
                UnlockInternal(UnlockType.Hold);
                return true;
            }
            catch (Exception e)
            {
...
#region State Updates
        ...
        private void OnClassifierEventUpdate(GattCharacteristic sender, GattValueChangedEventArgs args)
        {
            ...
            switch ((ClassifierEventType) eventType)
            {
                case ClassifierEventType.Pose:
                    ...
                    break;
                case ClassifierEventType.ArmSynced:
                    lock (_lock)
                    {
                ...
                case ClassifierEventType.Locked:
                    Unlocked = false;
                    break;
                case ClassifierEventType.ArmUnsynced:
                    Synced = false;
                    break;
            }
        }
        ...
        private async void ReadFirmwareVersion()
        {
...
#region Commands
        ...
        private void UnlockInternal(UnlockType type)
        {
            if (_myoBle != null)
            {
        ...
        private void NotifyUserActionInternal()
        {
            if (_myoBle != null)
            {
                _commandCharacteristic?.WriteValueAsync(new byte[]
                {
                    (byte) Command.UserAction,
                    1,
                    0
                }.AsBuffer());
            }
...
#region Utility
        ...
        {
#if WINDOWS_UWP
            UnlockInternal(UnlockType.Lock);
#endif
        }
